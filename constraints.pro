...
workspace_package_name(WorkspaceCwd, WorkspacePackageName) :-
  workspace_basename(WorkspaceCwd, WorkspaceBasename),
  atom_concat('@rabby-wallet/', WorkspaceBasename, WorkspacePackageName).
...
repo_name(RepoUrl, RepoName) :-
  Prefix = 'https://github.com/RabbyHub/',
...
% True if DependencyIdent starts with '@rabby-wallet' and ends with '-controller'
...
% must be called "@rabby-wallet/foo").
%
...
gen_enforced_field(WorkspaceCwd, 'name', WorkspacePackageName) :-
  WorkspaceCwd \= '.',
  \+ sub_atom(WorkspaceCwd, _, _, _, 'apps/'),
  workspace_package_name(WorkspaceCwd, WorkspacePackageName).
...
gen_enforced_field(WorkspaceCwd, 'keywords', ['RabbyHub', 'Ethereum']) :-
...
gen_enforced_field(WorkspaceCwd, 'homepage', CorrectHomepageUrl) :-
  ...
  repo_name(RepoUrl, RepoName),
  atomic_list_concat(['https://github.com/RabbyHub/', RepoName, '/tree/main/packages/', WorkspaceBasename, '#readme'], CorrectHomepageUrl).
...
gen_enforced_field(WorkspaceCwd, 'bugs.url', CorrectBugsUrl) :-
  ...
  workspace_field(WorkspaceCwd, 'repository.url', RepoUrl),
  repo_name(RepoUrl, RepoName),
  atomic_list_concat(['https://github.com/RabbyHub/', RepoName, '/issues'], CorrectBugsUrl).
...
% All packages must match the URL of a repo within the RabbyHub organization.
gen_enforced_field(WorkspaceCwd, 'repository.url', 'https://github.com/RabbyHub/core.git') :-
  workspace_field(WorkspaceCwd, 'repository.url', RepoUrl),
  \+ repo_name(RepoUrl, _).
