...
Originally forked from [MetaMask/providers](https://www.npmjs.com/package/@metamask/providers), Thx to [MetaMask](https://metamask.io).

=============================================================

The Ethereum provider object injected by Rabby into various environments.
Contains a lot of implementation details specific to Rabby, and is probably
...
`RabbyInpageProvider` further extends `StreamProvider` to support legacy provider interfaces in addition to [EIP-1193], and is used to instantiate the object injected by Rabby into web pages as `window.ethereum`.

## Usage

```javascript
import { initializeProvider } from '@rabby-wallet/providers';
...
The project follows the same release process as the other libraries in the Rabby organization:
