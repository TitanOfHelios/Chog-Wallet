...
var jobId = 0;
var nativeFSInstallAttempted = false;
...
function installNativeFS() {
  if (global.__RabbyNativeFS) {
    return true;
  }
  if (nativeFSInstallAttempted && !global.__RabbyNativeFS) {
    return false;
  }
  nativeFSInstallAttempted = true;

  if (typeof RNFSManager.install !== 'function') {
    return false;
  }

  try {
    const installed =
      RNFSManager.install() === true && !!global.__RabbyNativeFS;
    nativeFSInstallAttempted = installed;
    return installed;
  } catch (_error) {
    nativeFSInstallAttempted = false;
    return false;
  }
}
...
function getNativeFS() {
  if (!installNativeFS()) {
    throw new Error(
      '@rabby-wallet/react-native-fs JSI bindings are not available',
    );
  }
  return global.__RabbyNativeFS;
}

function requireByteInput(contents) {
...
var RNFS = {
  ...
  writeFileBytes(filepath: string, contents: Uint8Array | ArrayBuffer): void {
    getNativeFS().writeFileBytes(
      normalizeFilePath(filepath),
      requireByteInput(contents),
    );
  },

  appendFileBytes(filepath: string, contents: Uint8Array | ArrayBuffer): void {
    getNativeFS().appendFileBytes(
      normalizeFilePath(filepath),
      requireByteInput(contents),
    );
  },
  ...
  getDiagnosticsSnapshot() {
    return getNativeFS().getDiagnosticsSnapshot();
  },

  clearDiagnostics(): void {
    getNativeFS().clearDiagnostics();
  },

  existsSync(filepath: string): boolean {
    return getNativeFS().exists(normalizeFilePath(filepath));
  },
