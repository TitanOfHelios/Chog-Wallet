...
Local Rabby fork of `react-native-keychain@8.2.0`.

## Rabby Notes

- This package is the current business-facing keychain implementation for Rabby Mobile.
- It is based on the `react-native-keychain@8.2.0` source tree plus Rabby-specific compatibility and debug changes.
- Native module names are Rabby-prefixed so this package can coexist with the official `react-native-keychain` package in one app during migration research.
- Android storage compatibility is intentionally preserved for the existing `RN_KEYCHAIN` shared-preference data and per-service keystore aliases.

## Why It Exists

- Keep the current 8.2.0 runtime behavior under a workspace package we control.
- Let the app route normal business logic through the local fork.
- Keep the official `react-native-keychain@9.x` installed separately for investigation and upgrade experiments.
- Make it possible to eventually return to the community package if the Android upgrade path becomes safe.

## Upstream Baseline

- Source baseline: `react-native-keychain@8.2.0`
- License: MIT
