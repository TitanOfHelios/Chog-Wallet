...
export const SignTestnetTx = ({
  ...
  const {
    ...
    async (custom: number) => {
      return apiCustomTestnet.getCustomTestnetGasMarket({
        chainId,
        custom,
      });
    },
  ...
  const init = async () => {
    ...
    // }
    checkCanProcess();
    const balance = await apiCustomTestnet.getCustomTestnetToken({
      chainId,
      address: currentAccount.address,
    });
  ...
  const checkCanProcess = async () => {
    ...
    if (currentAccount.type === KEYRING_TYPE.GnosisKeyring || isGnosis) {
      setCanProcess(false);
      setCantProcessReason(t('page.signTx.multiSigChainNotMatch'));
    }
  };
