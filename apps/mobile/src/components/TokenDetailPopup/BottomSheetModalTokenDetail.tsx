...
type BottomSheetModalTokenDetailProps = {
  /** @internal */
...
export const BottomSheetModalTokenDetail = ({
  ref,
...
}: BottomSheetModalTokenDetailProps & {
  ...
  const onEndReached = React.useCallback(() => {
    ...
    if (latestData?.tokenId !== token?._tokenId) {
      if (__DEV__) {
        console.warn('latestData?.tokenId !== token?._tokenId, skip load more');
      }
      return;
    }
    loadMore();
  ...
  return (
    <AppBottomSheetModal
      ...
      <AutoLockView
        as="BottomSheetView"
        style={[styles.container, { paddingBottom: androidOnlyBottomOffset }]}>
