...
export const useTwoStepSwap = ({
  ...
  const next = useCallback(
    ...
    [shouldTwoStep, currentTxs, onApprovePending],
  );
  ...
  useEffect(() => {
    if (shouldTwoStep) {
      ...
      eventBus.addListener(EVENTS.INNER_HISTORY_ITEM_COMPLETE, complete);

      return () => {
        eventBus.removeListener(EVENTS.INNER_HISTORY_ITEM_COMPLETE, complete);
      };
    }
