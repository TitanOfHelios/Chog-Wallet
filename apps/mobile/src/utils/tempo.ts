...
const toRawAmountHex = (rawAmount: BigNumber, fallback = '0x0') => {
  if (!rawAmount.isFinite() || rawAmount.lt(0)) {
    return fallback;
  }
  const integerRawAmount = rawAmount.integerValue(BigNumber.ROUND_FLOOR);
...
export type TempoFeeTokenInfo = GasTokenInfo & {
  logoUrl: string;
...
export const isTempoTxType = (type: unknown) => {
  if (type === 118 || type === '0x76' || type === '0X76') {
    return true;
  }
...
export const isTempoSpecialTransaction = <T extends Record<string, unknown>>(
  ...
  if (isTempoTxType(tx.type)) {
    return true;
  }
  if (hasValidTempoCalls(tx.calls)) {
    return true;
  }
  if (hasValidTempoFeePayerSignature(tx.feePayerSignature)) {
    return true;
  }
  if (hasTempoFieldValue(tx.feeToken)) {
    return true;
  }
  if (hasTempoFieldValue(tx.nonceKey)) {
    return true;
  }
  if (hasTempoFieldValue(tx.keyAuthorization)) {
    return true;
  }
  if (hasTempoFieldValue(tx.validBefore)) {
    return true;
  }
  if (hasTempoFieldValue(tx.validAfter)) {
    return true;
  }
  return false;
};
...
export const toTempoCallsTx = <T extends TempoTxLike>(
  ...
  if (Array.isArray(nextTx.calls) && nextTx.calls.length > 0) {
    nextTx.calls = nextTx.calls.map(call => normalizeCall(call || {}, nextTx));
  } else {
...
const readAddressByMethod = async (params: {
  ...
  const { account, userAddress, chainServerId, functionName } = params;
  ...
  if (isSameTokenId(decoded, ZERO_ADDRESS)) {
    return null;
  }
...
export const getTempoFeeTokenAddress = async (params: {
  ...
  const { account, userAddress, chainServerId } = params;
...
const readErc20Balance = async (params: {
  ...
  const { account, tokenAddress, userAddress, chainServerId } = params;
...
const readErc20Decimals = async (params: {
  ...
  const { account, tokenAddress, chainServerId } = params;
...
export const getTempoFeeTokenInfo = async (params: {
  ...
  if (userAddress) {
    try {
      token = await openapi.getToken(userAddress, targetServerId, tokenId);
    } catch {
      token = null;
    }
  }
...
const createTempoTokenItem = (params: {
  ...
  return {
    ...
    raw_amount: normalizedRawAmount.isFinite()
      ? normalizedRawAmount.toFixed(0)
      : '0',
    raw_amount_hex_str: normalizedRawAmountHex.startsWith('0x')
      ? normalizedRawAmountHex
      : toRawAmountHex(normalizedRawAmount),
  };
};
...
export const listTempoFeeTokenOptionsFromCache = (params: {
  ...
  tokenList.forEach(token => {
    ...
    if (!TEMPO_FEE_TOKEN_WHITELIST_MAP.has(tokenId)) {
      return;
    }
...
export const listTempoFeeTokenOptions = async (params: {
  ...
  list.forEach(token => {
    ...
    if (!TEMPO_FEE_TOKEN_WHITELIST_MAP.has(key)) {
      return;
    }
