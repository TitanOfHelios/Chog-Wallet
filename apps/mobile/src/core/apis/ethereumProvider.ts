...
let sendRequestHandler: SendRequestHandler | null = null;

export function setEthereumProviderSendRequestHandler(
    ...
    switch (data.method) {
      ...
      case 'eth_sendTransaction': {
        if (!sendRequestHandler) {
          throw new Error('sendRequest handler is not ready');
        }
        const txParams = {
          ...data.params[0],
...
export const builtinEthereumProvider = new EthereumProvider();
